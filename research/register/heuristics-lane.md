...
| HL-7 Pre-election window tilt | long-bias the 3m into a general-election result | PBC literature (Nordhaus/Rogoff-Sibert/Shi-Svensson): visible spending + transfer pulses pre-election | **teach-only / paper-trade** (2026-09-02): FP1a passed as registered but deflates on dissection (result-month inclusion, 2009 carry, n=8); scored forward one election at a time; FP-D1 (clean window + global control) registered for the paper trade; L5 stays scheduling-only | open → paper trade |
