# The Oil/Energy Cycle — Fold Monograph (Atlas 2.12; L9's decomposed leg)

**Version 1.0 · 2026-09-02 · Ionic quant desk (principal: gaurav@ionic.in) · governed by research/CONTRACT.md**

**Verdict up front:** the "oil cycle" label RETIRES into regimes (OPEC eras + the shale break)
and one seated commitment: L9's oil leg consumes the KILIAN DECOMPOSITION, never the raw
price — the same $20 rise is a symptom in a boom and a tax in a bust, and a state that cannot
tell them apart is worse than no state. No new seat, no new budget.

**Headline result (OL1, pre-registered): the shock-flavor split, passed by 8x and CAPPED.**
Among oil-up years 1994-2015 (n=11), India averaged **+38.1%** in demand-flavored years vs
**−43.1%** in supply-flavored ones (+81.2pp vs a +10pp bar). The registered proxy — the global
equity sign — cannot separate "oil-up in a global bust" from "global bust", so the print is
largely GF2's global loading wearing an oil costume (2008's annual-averaging artifact noted).
It licenses the decomposition commitment and the shock-type briefing table; the measured
oil-SPECIFIC asymmetry waits for the real Kilian index (design OL-D1, expected to survive
but shrink under controls). The identification ladder is Part D-H's framework: equity-sign
shadow → Kilian IGREA → structural VAR.

**Assembled from:** partAB-theory-cases.md (Kilian at teaching depth incl. the elasticity-
bounds and Baumeister-Hamilton critiques and the IGREA correction episode; the OPEC/shale
regime chronology with verified dates; India's shock-type table incl. the post-2022
discounted-Russian-crude regime) · oil-RESULTS.md · partDH-verdict-routing.md.

---

# PART A + B + C + D — The decomposition, the regimes, India's transfer, the fold
...
# PART RESULTS — The desk's own numbers (OL1, pre-registered)

# Atlas 2.12 — oil fold: the shock-flavor asymmetry (OL1, pre-registered)

Oil-up years 1994-2015 (annual real oil return > +10%): n = 11 (8 demand-flavored, 3 supply-flavored — TINY, stated;
...
- Mean India return: demand-flavored **+38.1%**, supply-flavored **-43.1%**; difference **+81.2pp** (bar ≥ +10pp): **PASS**.

## Honest read (written AFTER the print)

- **OL1 passes its bar by 8x — and the dissection caps what it means.** The flavor proxy IS
  the global-equity sign, so "supply-flavored oil-up" years are by construction global-down
  years — and India (GF2 loading +0.57) falls in global-down years regardless of what oil is
  doing. The declared conflation does most of the work: this print is consistent with
  Kilian's asymmetry and nearly indistinguishable from the global factor's loading. Note also
  2008's annual-averaging artifact (H1 spike + H2 crash nets to an "oil-up year").
- **What the pass licenses:** the SHOCK-TYPE table as the briefing exhibit; the decomposition
  COMMITMENT (raw oil price never enters L9 — the same $20 means opposite things); the
  teaching claim that oil-up is not one object. **What it does not license:** any measured
  oil-SPECIFIC asymmetry — identification waits for the real Kilian index (runsheet), where
  supply/demand/precautionary shocks are separated by structure, not by the equity tape.
- The fold stands on design grounds either way: 2.12 was never a seat candidate; it is L9's
  decomposed leg plus the commodity monograph's capex machinery, under their existing budgets.

---

# Parts D–H — verdict + routing (atlas 2.12; fold into L9's decomposed leg)

## Part D — What OL1 establishes, and the identification ladder

OL1's +81.2pp is the project's clearest lesson in WHY identification discipline exists: a
massive, bar-clearing print whose registered proxy cannot distinguish "oil-up in a global
bust" from "global bust". The read caps it accordingly. The identification ladder for this
entry, in order of strength: (1) the equity-sign proxy (OL1 — direction-consistent, capped);
(2) the real Kilian index (structural identification via freight-proxied global activity —
runsheet); (3) full Kilian-Murphy/Baumeister-Hamilton VAR variants (literature-grade, not
desk machinery). L9 consumes level (2) when pulled; level (1) remains a briefing shadow. The
COMMITMENT the entry hardens: raw oil price NEVER enters L9 — the same $20 rise is a headwind
or a symptom, and a state that cannot tell them apart is worse than no state.

## Part E — Routing (the fold)

| Content | Where it lives |
|---|---|
| Oil supercycle / capex-lag / 2014 shale break | commodity monograph (H53 lane) — cross-ref |
| The decomposed oil state | L9's Kilian leg (seated in its role line since design) |
| Administered-price wedge (OMC pass-through) | fiscal/mp monographs + commodity partC — cross-ref |
| The shock-type India table | Stage-2 briefing exhibit (this entry's own artifact) |
| "The oil cycle" label | RETIRED — regimes (OPEC eras, shale era), not a cycle; print attached |

## Part F — Harvest + designs

Harvest: the shock-type briefing table; the decomposition commitment with OL1's capped print
as its teaching case; the OPEC-regime chronology as breaks-registry entries for any oil-linked
series. Designs: **OL-D1** the Kilian-index pull + replication of OL1 with structural flavors
(acceptance registered at pull; the desk expects the asymmetry to SURVIVE but shrink once the
global factor is controlled). **OL-D2** the windfall asymmetry (supply-driven oil-DOWN years —
2014-16 — as the reverse experiment; design only, n tiny).

## Part H — Knowledge ledger (atlas 2.12)

**Established (our runs):** oil-up years split violently for India by global context (+38 vs
−43%, n=11) — direction-consistent with Kilian, identification capped and stated.
**Established (record):** oil's "cycle" is OPEC/shale REGIMES on a 5-10y investment lag whose
short end the shale era flattened; India's transfer flipped sign twice as an experiment
(2013 stress, 2014-16 windfall). **Unknowable:** the next OPEC+ regime's shape and Hormuz-
